#!/usr/bin/env python3
"""
Preview merged image and ask user if they want to recapture
"""
...
def main():
    if len(sys.argv) < 4:
        print("Usage: preview-and-confirm.py <image_path> <description> <capture_count>", file=sys.stderr)
        sys.exit(1)

    image_path = sys.argv[1]
    description = sys.argv[2]
    capture_count = int(sys.argv[3])

    try:
        window = PreviewWindow(image_path, description, capture_count)
        recapture = window.show()
        ...
        sys.exit(0 if not recapture else 1)  # Exit code 1 means recapture
